...
# Initialize AerSimulator backend for scalable execution under Qrisp environment
...
def create_qrisp_state_prep(trotter_unitaries, n_qubits):
    ...
    def state_prep(params):
        ...
        # Apply pre-trotterized unitary generators onto the quantum variable
...
def compute_statevector_1d(params, trotter_unitaries, n_qubits):
    """
    Executes quantum state synthesis in Qrisp and extracts the full statevector array.
    """
    qv = qrisp.QuantumVariable(n_qubits)
    for j, U in enumerate(trotter_unitaries):
        if U is not None:
...
def compute_qfim_sparse(params, trotter_unitaries, n_qubits, eps=1e-5):
    """
    Computes Quantum Fisher Information Matrix (QFIM) g_μν using finite difference 
    statevector differentiation (central difference approximation):
    ∂_j |ψ⟩ ≈ (|ψ(θ + ε e_j)⟩ - |ψ(θ - ε e_j)⟩) / (2ε).
    """
    m = len(params)
    psi = compute_statevector_1d(params, trotter_unitaries, n_qubits)

    # Initialize Jacobian matrix J where columns correspond to state derivatives w.r.t. parameters
    ...
    # Construct metric matrix g = Re( J^† J - (J^† |ψ⟩)(⟨ψ| J) )
    v = J.conj().T @ psi
    g = np.real((J.conj().T @ J) - np.outer(v, v.conj()))
    # Enforce symmetric matrix structure
    return 0.5 * (g + g.T)


def compute_gradient_parameter_shift(ev_H, params):
    """
    Computes the exact energy gradient ∇E(θ) using the Quantum Parameter-Shift Rule:
    ∂E / ∂θ_j = E(θ + π/4 e_j) - E(θ - π/4 e_j).
    Valid for unitaries generated by Pauli string operators with binary spectrum ±1.
    """
...
def qng_step_qrisp(params, H_op, trotter_unitaries, n_qubits, lr=0.08, eps=1e-3, max_grad_norm=0.5):
    """
    Executes one Quantum Natural Gradient (QNG) step in Qrisp using parameter-shift gradients 
    and statevector QFIM metric inversion with dynamic Tikhonov regularization.
    """
    state_prep = create_qrisp_state_prep(trotter_unitaries, n_qubits)
    # Bind expectation value estimator to the current state preparation circuit
    ev_H = H_op.expectation_value(state_prep)

    current_E = float(ev_H(params))
    # Evaluate exact parameter gradient via parameter shift
    ...
    current_eps = eps
    # Adaptive regularization loop for inverse matrix stabilization
    for _ in range(6):
        ...
        # Clip maximum step size in metric space to prevent optimization divergence
        ...
        # Accept step if energy strictly decreases or stays within tolerance threshold
        if candidate_E <= current_E + 1e-12:
            return candidate_params, candidate_E, grad_norm

        # Increase regularization if state update produced an energy increase
        current_eps *= 4.0

    # Fallback step: standard steepest descent gradient move
...
def self_verifying_AVQE_qrisp(
    ...
    """
    Scalable, native Qrisp implementation of Self-Verifying AVQE.
    ...
    # -------------------------------------------------------
    # Automatically build trotterized Pauli rotation gates
    # -------------------------------------------------------
    trotter_unitaries = []
    for p_str in P_strings:
        ...
        # Construct trotterized unitary gate object exp(-i * t * term)
        trotter_unitaries.append(
            term.trotterization() if term is not None else None
        )

    # Define Hamiltonian difference operator ΔH = H_f - H_i used for verification step size bound
    H_diff = H_f - H_i
    state_prep = create_qrisp_state_prep(trotter_unitaries, n_qubits)

    lbd = 0.0
    np.random.seed(42)
    # Add random Gaussian noise to break initialization symmetry
    ...
    # Outer adiabatic progression loop over schedule parameter λ ∈ [0, 1]
    while lbd < 1.0:

        step += 1

        # Construct instantaneous linear adiabatic Hamiltonian H(λ) and squared operator H²(λ)
        H_curr = ((1.0 - lbd) * H_i + lbd * H_f).hermitize()
        H2_curr = (H_curr * H_curr).hermitize()

        # Bind expectation value callables to the current state preparation function
        ...
        while True:
            ...
            # Measure instantaneous expectation value ⟨H⟩ and second moment ⟨H²⟩
            E_val = float(np.real(ev_H(params)))
            E2_val = float(np.real(ev_H2(params)))
            # Compute energy standard deviation σ_ψ(H) = √(⟨H²⟩ - ⟨H⟩²)
            ...
            # Benchmarking: Compute sparse exact ground state and infidelity 1 - F
            if track_exact:
                ...
                # Solve lowest eigenvalue/eigenvector using SciPy ARPACK sparse solver
            ...
            # Certificate check: Exit inner loop if energy variance satisfies σ_ψ(H) < Δ_C / 2
            if sigma_H < delta_C / 2:
                stagnant_checks = 0
                break

            # Escape mechanism: apply random parameter kick if optimizer plateaus prematurely
        ...
        # Evaluate standard deviation of operator difference ΔH = H_f - H_i
        ...
        # Dynamic step size calculation bound δλ_V ensuring ground-state fidelity retention
        if sigma_delta < 1e-9:
            dl_V = dl_A
        else:
            dl_V = (delta_C / 2 - sigma_H) / sigma_delta

        # Select adaptive adiabatic step size δλ = min(δλ_A, δλ_V, 1 - λ)
        dl = min(
            dl_A,
            max(0.005, dl_V),
            1.0 - lbd
    ...
    # Reconstruct state preparation circuit and return parameters and final quantum session
